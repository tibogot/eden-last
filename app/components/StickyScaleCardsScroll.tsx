...
export default function StickyScaleCardsScroll({
  ...
  return (
    <div className={`w-full ${className}`} ref={container}>
      {introHeadline && (
        <section className="flex min-h-svh flex-col items-center justify-center bg-secondary px-6 py-16 md:px-8">
      ...
      <section className="sticky-scale-cards-section flex min-h-svh items-center justify-center bg-secondary text-primary">
        <div className="sticky-scale-cards-container relative h-[50vh] w-[95%] overflow-hidden md:w-1/2">
          {cards.map((item, index) => (
            ...
            >
              <div className="absolute left-4 top-4 z-10 rounded bg-primary px-2 py-1">
                <p className="font-neue-haas text-secondary text-xs font-semibold uppercase leading-none tracking-wide">
                  {item.tag}
                </p>
              </div>
