...
export default function OverlappingCardSwiper({
  ...
  useEffect(() => {
    ...
    const totalWidth =
      slides.length * cardWidth + (slides.length - 1) * gap;
  ...
  return (
    <section
      className={`flex w-full items-center gap-[2vw] overflow-hidden ${className}`}
      style={{ minHeight: "80vh" }}
    >
      {title ? (
        <div className="flex w-1/2 shrink-0 flex-col items-start justify-center px-[4vw]">
          {title}
        </div>
      ) : null}
      <div
      ...
      >
        ...
        >
          {slides.map((slide, index) => (
            <div
              key={slide.id ?? index}
              className="pointer-events-none flex shrink-0 flex-col justify-between rounded-[2vw] border-2 border-black/60 p-[2vw]"
              style={{
                width: `${widthVw}vw`,
                height: `${heightVw}vw`,
                backgroundColor:
...
export const SAMPLE_OVERLAPPING_SLIDES: OverlappingSlide[] = [
  {
    ...
    content: (
      <>
        <p className="font-neue-haas text-base font-medium leading-tight text-black md:text-lg">
